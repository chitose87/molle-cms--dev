      ...
      .then((snap: firebase.firestore.DocumentSnapshot) => {
        ...
        if (itemData.type == "children" || itemData.type == "group") {
          ...
          for (let i in itemData.value) {
            ...
            n = Number( i );
            this.loop(itemData, n, () => {
              console.log("loop呼んだあと",itemData)
              let complete:any = itemData;

              let createLoop2 = (_itemData:any) => {
                console.log("for文２の直前")
                let m: number;
                if (_itemData.type == "children" || _itemData.type == "group") {
                  for (let j in _itemData.value) {
                    console.log("for文２の1行目",j)
                    m = Number( j );
                    console.log("for文２でchildrenの時（createLoop2を呼ぶところ）")
                    createLoop2(_itemData.value[m].obj);
                  }
                }
                console.log("objを削除するところ")
                delete itemData.obj;
                itemData.createTime = this.updateTime;
                // this.batch.create(Singleton.itemsRef.doc(childNode.id),itemData);
              };
              console.log("createLoop2を最初に呼ぶところ")
              createLoop2(complete);
            });
          }
        } else {
