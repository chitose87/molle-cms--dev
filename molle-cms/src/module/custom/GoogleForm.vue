      ...
      table.contact-form__fieldset
        tbody
          tr.contact-form__row(
            ...
            td.contact-form__body(v-else-if="item.type === 2")
              // ラジオボタン
              label.contact-form__label(v-for="option in JSON.parse(item.typeOption)")
                input.contact-form__item(
                  type="radio",
                  v-model="arg[`entry.${item.id}`]",
                  :name="item.name",
            ...
            td.contact-form__body(v-else-if="item.type === 9")
              // 日付
              input.contact-form__item(
                type="date",
                v-model="arg[`entry.${item.id}`]",
